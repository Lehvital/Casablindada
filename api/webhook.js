// api/webhook.js - Serverless Function para Vercel
export default async function handler(req, res) {
  ...
  try {
    ...
    if (whatsapp && process.env.ZAPI_INSTANCE && process.env.ZAPI_TOKEN) {
      fetch(`https://api.z-api.io/instances/${process.env.ZAPI_INSTANCE}/token/${process.env.ZAPI_TOKEN}/send-text`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          ...
          message: `🎉 Venda confirmada! Acesse seu produto: ${process.env.PRODUCT_LINK}`
        })
      }).catch(() => {});
    }
